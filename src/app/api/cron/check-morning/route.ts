...
/**
 * Chạy sau 1 giờ kể từ giờ uống thuốc buổi sáng.
...
async function run() {
  ...
  for (const med of meds) {
    ...
    // Chỉ nhắc nếu chưa uống liều sáng
    if (!log.morningTaken) {
      missing.push(`${med.name} (liều sáng ${med.morningTime})`);
    }
  }
  ...
  if (missing.length > 0) {
    notified = await notifyAll({
      title: "⏰ Nhắc nhở: Bạn chưa uống thuốc buổi sáng!",
  ...
  return { 
    ...
    message: missing.length > 0 
      ? `Đã gửi nhắc nhở cho ${missing.length} liều sáng`
      : "Đã uống đủ liều sáng"
  };
}
